...
func TestParseSshHost(t *testing.T) {
	...
	if err != nil || host.Channel != "master" || host.Username != "root" || host.URI != "tcp://loc.m:22" {
		fmt.Println(err)
		t.Error(host)
		return
	}
	host, err = ParseSshHost("abc", "root:sco@loc.m", nil)
	if err != nil || host.Channel != "master" || host.Username != "root" || host.URI != "tcp://loc.m:22" || host.Password != "sco" {
		fmt.Println(err)
		t.Error(host)
		return
	}
	host, err = ParseSshHost("abc", "mx://root:sco@loc.m", nil)
	if err != nil || host.Channel != "mx" || host.Username != "root" || host.URI != "tcp://loc.m:22" || host.Password != "sco" {
		fmt.Println(err)
		t.Error(host)
		return
	}
	...
	if err != nil || host.Channel != "mx" || host.Username != "root" ||
		host.URI != "tcp://loc.m:22" || host.Password != "sco" ||
		host.Pty != "vt100" {
		fmt.Println(err)
		t.Error(host)
		return
